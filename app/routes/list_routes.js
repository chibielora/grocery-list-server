...
// pull in Mongoose model for list
...
router.get('/list', requireToken, (req, res, next) => {
  List.find()
    .then(lists => {
      // `list` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
      // apply `.toObject` to each one
      return lists.map(list => list.toObject())
    })
    // respond with status 200 and JSON of the list
...
// SHOW
// GET /list/5a7db6c74d55bc51bdf39793
...
// UPDATE
// PATCH /list/5a7db6c74d55bc51bdf39793
...
// DESTROY
// DELETE /list/5a7db6c74d55bc51bdf39793
